...
fn map_market(value: Value) -> Result<Option<Market>> {
    ...
    let resolution_criteria = first_string(
        &value,
        &[
            "resolutionCriteria",
            "resolution_criteria",
...
#[cfg(test)]
mod tests {
    use super::{map_market, parse_market_spec};
    ...
    #[test]
    fn map_market_keeps_top_level_rule_text_fields() {
        let market = map_market(json!({
            "id": "mkt-1",
            "slug": "nyc-high-apr-2",
            "question": "Will the high temperature in New York on April 2, 2026 be above 20 C?",
            "description": "Top-level description.",
            "resolutionCriteria": "Use the official station high.",
        ...
        assert_eq!(
            market.source_url.as_deref(),
            Some("https://example.com/rules")
        );
    }

    #[test]
    fn map_market_falls_back_to_event_rule_text_fields() {
        let market = map_market(json!({
            "id": "mkt-2",
            "slug": "nyc-high-apr-2-fallback",
            "question": "Will the high temperature in New York on April 2, 2026 be above 20 C?",
            "tokens": [
                { "outcome": "Yes", "tokenId": "yes-2" },
                { "outcome": "No", "tokenId": "no-2" }
            ],
            "events": [
                {
                    "description": "Event-level description.",
                    "rules": "Resolve using the linked source.",
                    "sourceUrl": "https://example.com/event-rules"
                }
            ]
        }))
        .expect("map market should succeed")
        ...
        assert_eq!(
            market.source_url.as_deref(),
            Some("https://example.com/event-rules")
        );
    }
}
