...
use domain_core::{ForecastBundle, LlmInsight, Market, PosteriorEstimate, SignalSide, TradeSignal};
...
async fn run_cycle(
    ...
    for market in &markets {
        ...
        let llm_insight = compose_llm_insight(&llm, market, &merged_forecast, &posterior).await?;
...
async fn compose_llm_insight(
    llm: &LlmAnalyst,
    market: &Market,
    forecast: &ForecastBundle,
    posterior: &PosteriorEstimate,
) -> Result<Option<LlmInsight>> {
    let pre_trade = llm.analyze_pre_trade(market, forecast, posterior).await?;
    let rule_text = build_market_rule_text(market);
    let rule_parse = if rule_text.trim().is_empty() {
        None
    } else {
        llm.parse_market_rules(market, &rule_text).await?
    };
    let divergence = llm
        .explain_forecast_divergence(market, forecast, posterior)
        .await?;

    Ok(merge_llm_outputs(market, pre_trade, rule_parse, divergence))
}

fn build_market_rule_text(market: &Market) -> String {
    let mut sections = Vec::new();

    if !market.question.trim().is_empty() {
        sections.push(format!("Question: {}", market.question.trim()));
    }
    if let Some(description) = market.description.as_deref() {
        let description = description.trim();
        if !description.is_empty() {
            sections.push(format!("Description: {description}"));
        }
    }
    if let Some(resolution_criteria) = market.resolution_criteria.as_deref() {
        let resolution_criteria = resolution_criteria.trim();
        if !resolution_criteria.is_empty() {
            sections.push(format!("Resolution criteria: {resolution_criteria}"));
        }
    }
    if let Some(source_url) = market.source_url.as_deref() {
        let source_url = source_url.trim();
        if !source_url.is_empty() {
            sections.push(format!("Source URL: {source_url}"));
        }
    }
...
fn merge_llm_outputs(
    market: &Market,
    pre_trade: Option<LlmInsight>,
    rule_parse: Option<llm_core::RuleParseInsight>,
    divergence: Option<llm_core::ForecastDivergenceInsight>,
) -> Option<LlmInsight> {
    match (pre_trade, rule_parse, divergence) {
        (None, None, None) => None,
        (Some(insight), None, None) => Some(insight),
        (None, Some(rule), None) => Some(LlmInsight {
            market_id: market.market_id.clone(),
            summary: join_summary_segments(&[format!("Rule parse: {}", rule.summary)]),
            caution_flags: merge_caution_flag_groups(&[&rule.caution_flags]),
        }),
        ...
        (Some(insight), Some(rule), None) => Some(LlmInsight {
            ...
            summary: join_summary_segments(&[
                insight.summary,
                format!("Rule parse: {}", rule.summary),
            ]),
            caution_flags: merge_caution_flag_groups(&[
                &insight.caution_flags,
                &rule.caution_flags,
            ]),
        }),
        (Some(insight), None, Some(divergence)) => Some(LlmInsight {
            market_id: insight.market_id,
            summary: join_summary_segments(&[
                insight.summary,
                format!("Forecast divergence: {}", divergence.summary),
            ]),
            caution_flags: merge_caution_flag_groups(&[
                &insight.caution_flags,
                &divergence.caution_flags,
            ]),
        }),
        (None, Some(rule), Some(divergence)) => Some(LlmInsight {
            market_id: market.market_id.clone(),
            summary: join_summary_segments(&[
                format!("Rule parse: {}", rule.summary),
                format!("Forecast divergence: {}", divergence.summary),
        ...
        (Some(insight), Some(rule), Some(divergence)) => Some(LlmInsight {
            market_id: insight.market_id,
            summary: join_summary_segments(&[
                insight.summary,
                format!("Rule parse: {}", rule.summary),
...
fn join_summary_segments(parts: &[String]) -> String {
    parts
        .iter()
        .map(|part| part.trim())
...
fn merge_caution_flag_groups(groups: &[&[String]]) -> Vec<String> {
